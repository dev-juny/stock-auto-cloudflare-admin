...
export function DrawdownChart() {
    ...
    useEffect(() => {
        ...
        if (trades.length > 0) {
            ...
            sorted.forEach((t) => {
                ...
                ddData.push({ time: dayStr, value: Math.min(((cumPnl - peak) / peak) * 100, 0) });
            });
            hist.setData(ddData);
        }
        const handleResize = () => {
    ...
    return (_jsxs(Card, { children: [_jsx("h2", { className: "text-sm font-semibold text-text-primary mb-2", children: "Drawdown" }), loading ? _jsx(CardSkeleton, {}) : _jsx("div", { ref: chartRef, className: "w-full", style: { height: 140 } })] }));
}
