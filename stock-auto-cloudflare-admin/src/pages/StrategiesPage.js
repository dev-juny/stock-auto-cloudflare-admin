...
function PromotionSection({ data, loading, onRefresh }) {
    ...
    return (_jsxs("div", { className: "space-y-2", children: [data.map(p => {
                const actionLabel = p.action || (p.new_status === 'production' ? 'promoted' : 'demoted');
                const isPromoted = actionLabel === 'promoted';
                return (_jsxs("div", { className: "flex items-center justify-between bg-surface rounded-xl px-3 py-2.5 text-xs", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: `px-1.5 py-0.5 rounded text-[10px] font-medium ${isPromoted ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`, children: actionLabel }), _jsxs("div", { children: [_jsx("div", { className: "text-text font-medium", children: p.strategy_name || `#${p.strategy_id}` }), p.reason && _jsx("div", { className: "text-text-muted text-[10px]", children: p.reason })] })] }), _jsxs("div", { className: "text-right", children: [_jsxs("div", { className: "text-amber-400", children: ["fitness: ", (p.fitness ?? 0).toFixed(2)] }), _jsx("div", { className: "text-text-muted text-[10px]", children: formatKST(p.promoted_at || p.created_at) })] })] }, p.id));
            }), _jsx("button", { onClick: autoPromote, disabled: actionLoading, className: "w-full text-xs px-3 py-2 rounded-lg bg-primary text-white font-medium hover:bg-primary/90 transition-colors disabled:opacity-50", children: actionLoading ? 'Running...' : 'Auto Promote Candidates' })] }));
}
function ValidationSection({ data: data_, loading, onRefresh }) {
    ...
    return (_jsxs("div", { className: "space-y-3", children: [_jsxs("div", { className: `flex items-center gap-2 p-3 rounded-xl text-xs font-medium ${data.is_active ? 'bg-blue-500/10 text-blue-400' : 'bg-amber-500/10 text-amber-400'}`, children: [_jsx(Activity, { size: 14 }), data.is_active
                        ...
                        : 'Inactive — Start 30-day validation to collect readiness data', data.started_at && _jsxs("span", { className: "text-text-muted", children: ["since ", formatKST(data.started_at)] })] }), data.is_active && data.today && (_jsxs("div", { className: "grid grid-cols-3 gap-2 text-xs", children: [_jsxs("div", { className: "bg-surface rounded-xl p-3 text-center", children: [_jsx(Tooltip, { content: findGlossary('return')?.description ?? 'Total Return', children: _jsx("div", { className: "text-text-muted text-[10px]", children: "Total Return" }) }), _jsxs("div", { className: `font-bold ${(data.today.cumulative_return ?? 0) >= 0 ? 'text-green-400' : 'text-red-400'}`, children: [(data.today.cumulative_return ?? 0) >= 0 ? '+' : '', (data.today.cumulative_return ?? 0).toFixed(2), "%"] })] }), _jsxs("div", { className: "bg-surface rounded-xl p-3 text-center", children: [_jsx(Tooltip, { content: findGlossary('mdd')?.description ?? 'MDD', children: _jsx("div", { className: "text-text-muted text-[10px]", children: "MDD" }) }), _jsxs("div", { className: "font-bold text-red-400", children: [(data.today.mdd ?? 0).toFixed(1), "%"] })] }), _jsxs("div", { className: "bg-surface rounded-xl p-3 text-center", children: [_jsx(Tooltip, { content: findGlossary('winRate')?.description ?? 'Win Rate', children: _jsx("div", { className: "text-text-muted text-[10px]", children: "Win Rate" }) }), _jsxs("div", { className: "font-bold text-blue-400", children: [(data.today.win_rate ?? 0).toFixed(1), "%"] })] })] })), _jsx("button", { onClick: toggleValidation, disabled: actionLoading, className: "w-full text-xs px-3 py-2 rounded-lg bg-primary text-white font-medium hover:bg-primary/90 transition-colors disabled:opacity-50", children: actionLoading ? 'Processing...' : data.is_active ? 'Stop Validation' : 'Start 30-Day Validation' })] }));
}
