    ...
    async function load() {
        try {
            const [d, evo, marketDate] = await Promise.all([
                ...
                api.get('/api/scheduler/market/latest-trade-date').catch(() => null),
            ]);
            const jobs = d.jobs || [];
            if (marketDate?.latest_trade_date) {
                const marketJob = jobs.find(j => j.job_id === 'market_data_sync');
                if (marketJob)
                    marketJob.latest_trade_date = marketDate.latest_trade_date;
            }
            setJobs(jobs);
            setEvoInfo(evo);
        }
