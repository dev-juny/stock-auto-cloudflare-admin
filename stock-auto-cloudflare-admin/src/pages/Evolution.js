...
export function EvolutionPage() {
    ...
    return (_jsxs("div", { className: "space-y-4", children: [_jsx(EvolutionHeader, { status: status, onRun: handleRun, onRefresh: fetchData }), _jsx(LiveStatus, { status: status, generationCount: generations.length, strategyCount: strategies.length }), _jsxs("div", { className: "bg-surface rounded-xl p-3 flex items-start gap-2.5", children: [_jsx(Info, { size: 14, className: "text-primary mt-0.5 shrink-0" }), _jsxs("div", { className: "text-[11px] text-text-muted leading-relaxed", children: [_jsx("strong", { className: "text-text", children: "Strategy Parameter Optimization" }), " \u2014 Evolves entry/exit parameters (entry type, stop-loss, take-profit, trailing stop) on a shared evaluation universe.", _jsx("strong", { className: "text-text", children: " Stock selection evolution is not implemented." }), " All strategies in a generation are tested against the same random sample of 50 tickers."] })] }), _jsxs("div", { className: "flex items-start sm:items-center justify-between border-b border-surface-border pb-2 gap-2", children: [_jsx("div", { className: "flex gap-2 flex-wrap", children: ['strategies', 'timeline'].map((t) => (_jsx("button", { onClick: () => setTab(t), className: `px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${tab === t ? 'bg-primary text-white' : 'text-text-muted hover:text-text'}`, children: t === 'strategies' ? 'Strategy Pool' : 'Evolution Timeline' }, t))) }), tab === 'timeline' && (_jsxs("div", { className: "flex items-center gap-2", children: [compareMode && (_jsxs("button", { onClick: handleCompareClick, disabled: compareSelections.size !== 2, className: `flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-full transition-colors ${compareSelections.size === 2
